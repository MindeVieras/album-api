...
const expectedBucket = config.bucket
const expectedHostname = 'http://'+config.bucket+'.s3.amazonaws.com'
const expectedMinSize = null
const expectedMaxSize = null

module.exports = function(app) {

  // app.post('/api/s3upload', Auth.isAuthed, Upload.upload)
  app.post('/api/s3upload', function(req, res) {
...
// app.delete("/s3upload/*", function(req, res) {
...
function signV2RestRequest(headersStr) {
    return getV2SignatureKey(config.client_secret_key, headersStr);
}
...
function signV4RestRequest(headersStr) {
    ...
    return getV4SignatureKey(config.client_secret_key, matches[1], matches[2], "s3", stringToSign);
}
...
function signV2Policy(base64Policy) {
    return getV2SignatureKey(config.client_secret_key, base64Policy);
}
...
function signV4Policy(policy, base64Policy) {
    ...
    return getV4SignatureKey(config.client_secret_key, matches[1], matches[2], "s3", base64Policy);
}
